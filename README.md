...
4.3.10 追加伦理误收修复，版本号保持不变：

- 伦理专区仅依据 `Tags` 中明确的归类标签，或 `Tags/Genres` 中完整的情色题材值。
- 简介中的“长大成人”、家庭伦理、厂牌名、父目录，以及单纯的 `18+`、`R18`、`NC-17` 分级不再触发入选。
- 在 Emby 条目的标签中填写 `伦理` 可确认加入；填写 `排除伦理` 可强制排除，排除优先。支持简繁体标签。下一次同步时生效。
- 动画、电影、剧集采用同一证据规则，不按片名设置黑名单，也不排除全部动画。
- 重建会重新计算伦理成员并移除旧规则误收的条目；只移除虚拟库引用。

严格规则可能减少自动命中数量。没有明确题材信息时暂不收录，可补充 Emby 标签；若 Emby 原有标签本身错误，仍需纠正标签或使用 `排除伦理`。泛指“伦理”的 **Genres 类型**不作为情色证据，独立的 **Tags 标签** `伦理` 视为明确归类。

下列完整题材值可自动入选：`情色`、`情色片`、`情色电影`（含繁体）、`erotic`、`erotica`、`softcore`、`sexploitation`、`pink film`、`roman porno`、`jav`。匹配不区分英文大小写，兼容字符串和带 `Name` 的标签对象。不会拆分长句做关键词搜索。

此次针对截图修复伦理规则；其余平台和榜单识别仍沿用此前 4.3.10 逻辑，尚未实施“ID 冲突禁止标题回退”等后续方案。
...
- 4.3.9 原先使用分级、类型、标签、标题、路径和简介关键词；本次已改为上述严格伦理规则。
