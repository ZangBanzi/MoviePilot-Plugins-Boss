...
4.3.9 在 4.3.8 基础上加入 `伦理专区`，并保持原来的 8098 访问链路：

- 新增 `伦理专区`，会成为 Emby 首页一级虚拟库，不放入合集。
- 支持电影和剧集；同一 ItemId 只出现一次，原媒体库位置不变。
- 优先识别 Emby 分级、类型、标签；再使用标题、路径和简介关键词兜底。
- 命中关键词包含 `伦理/倫理`、`成人`、`adult`、`erotic`、`R18`、`NC-17`、`18+`、`三级/三級`、`JAV` 等。
- 避免把 `AV1` 视频编码误判为成人内容；`AV` 不作为普通全文关键词。
- 伦理专区增加独立动态封面主题，首页不使用灰色默认封面。
...
1. 在 GitHub 提交新版文件；在 MoviePilot 插件市场刷新并升级至 **4.3.9**。
...
| 字段 | 含义 |
|---|---|
| `version` | 必须为 `4.3.9`；缺少此字段不能证明新版已加载 |
