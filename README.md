...
新增 **银幕、胶片、三联、刊物、黑胶、拼色** 六款构图：全幅剧照、六格海报长廊、三联画、纸感影像特刊、唱片画面及四宫拼贴。原有叠影、光幕、映墙、留白继续保留。

![十款实际绘图对照，使用原创示例素材](docs/previews/template-gallery.jpg)

- 参考 GitHub 的 [EmbyLibraryCover](https://github.com/g-steven037/MoviePilot-Plugins/blob/main/plugins.v2/embylibrarycover/renderer.py)、[Yahaha Cover Studio](https://github.com/justzerock/MoviePilot-Plugins) 等项目研究构图，由本项目独立绘制。未复制第三方代码或电影海报，来源见致谢。
- 十款方案均支持原生与虚拟库，生成时只使用各自有权限的影片素材；方案选择缩略图使用原创几何示例。新旧模板共用静态输出、真实 GIF 轮播、缓存、方案保存和历史恢复。
- 推荐银幕使用横版 Backdrop；胶片、三联、刊物使用竖版 Primary。切换模板保留当前素材来源、排序、标题、字体、分辨率和动画开关，仅恢复该款构图及配色；已有保存方案不会自动改变。
- 方案列表可滚动浏览，手机为两列。五款新方案使用固定版式，支持文字、字号、字体、配色和素材调整；银幕还可移动标题。界面仅显示适用的控件，刊物、黑胶和拼色的长标题优先分为两行，必要时缩小或省略以保护构图。

## 保留 4.5.2 方案选择与历史管理

- 媒体库支持 **全选、反选、逐库勾选**。勾选决定批量处理范围，点击卡片的编辑区域调整画布；切换媒体库会保留页面内未保存的修改。
- 默认 **统一使用当前方案**：选择“光幕”等方案后，“预览已选库”和“生成并应用已选库”都按当前参数处理所有勾选库。生成前一次保存选中库方案并固定本次参数，未勾选库保留原设置。统一方案包含字体和标题；主标题留空时分别使用各库名称。
- 也可选择 **使用各库已保存方案**，保留各库独立设计；草稿不会自动应用，需要先保存单库方案。卡片显示实际将生成的方案。
- 历史卡片可直接删除单张；**清空全部历史**覆盖所有服务器、全部批次，包含页面未展示的记录和原生封面备份。点击后确认再清理，保留 Emby 已应用封面、方案设置、字体和配置备份；生成或恢复期间禁用清理。文件删除失败可重试，即使索引已空也可“清理历史残留”。

[批量选择截图](docs/previews/studio-selection.png) · [手机选择截图](docs/previews/studio-selection-mobile.png) · [历史清理截图](docs/previews/studio-history-cleanup.png)
...
界面参考 [justzerock/MoviePilot-Plugins](https://github.com/justzerock/MoviePilot-Plugins/tree/main) 的呀哈哈封面工坊与用户提供的截图：深色卡片、蓝色分段导航、大画布、方案网格和分组配置。页面与 Python 绘图代码为本项目实现；来源和字体许可见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。

![工坊实际浏览器截图](docs/previews/studio-server.png)

- **十种封面**：叠影、光幕、映墙、留白、银幕、胶片、三联、刊物、黑胶、拼色。方案缩略图和大画布使用同一个真实绘图器。
...
- **历史封面**：当前库/勾选库后台生成、进度、停止、预览、原图下载、恢复方案、卡片删除和全部清理。默认保留 30 批，可设 1–100 批，图片总量上限 256 MiB；页面展示最近 60 张。
...
主选择框以已配置的 Emby 服务器为单位。先勾选目标；全选即可处理整台服务器。“预览已选库”逐库读取素材但不写入 Emby；“生成并应用已选库”按选择的方案模式生成，先备份再更新所选原生库图片，同时生成所选虚拟库历史并保存其播放端方案。点选卡片的编辑区域可调整单库，同名库显示 ID。服务器范围沿用插件已选择的 Emby；不会因封面操作切换播放网关。

两类库共用十种布局、字体、海报优先级、Backdrop→Primary 回退、图片解码与去重。虚拟库保留图片标签并先从全部成员中挑选有图候选；原生库还通过 Emby ImageTypes 过滤寻找靠后的有图条目，避免前排缺图就直接放弃。图片请求仍有数量/时间/大小上限，源站超时或素材确实不可用时显示品牌画面及提示。
...
截图和 GIF 使用构造元数据/原创示例图，不包含用户影片。新增诊断截图使用本地固定状态展示空库、单图和备份失败。此前曾对 NAS 做 4.4.1 的只读核验；用户本次截图显示已安装 4.5.0。本次 4.6.0 未连接或改动 NAS。4.3.11 的完整历史说明见 [docs/HISTORY-4.3.11.md](docs/HISTORY-4.3.11.md)。
...
本地发布包：`releases/MediaArchiver-v4.6.0.zip`。解压后按原目录上传文件，不要只把 ZIP 放进仓库。
...
2. 更新根目录 **`package.v2.json`** 的 `MediaArchiver` 条目到 **4.6.0**；仓库有其他插件时保留它们。同步图标、说明、字体许可与校验文件。`frontend/` 是可复现源码，NAS 运行不需要 Node。
3. 刷新 MoviePilot 插件市场并升级，使 MP 安装插件依赖。随后重启 MoviePilot，确保旧网关代码退出。
4. 检查 `http://NAS地址:3334/__mediaarchiver__/health`：`version` 和 `cover_engine` 应为 `4.6.0`，`code_sha256` 对照 `SHA256SUMS` 中主文件；`decoders.br/zstd` 应启用。
...
另一个终端在项目根目录执行 `node tests/ui_smoke.mjs`、`node tests/ui_output_diagnostics.mjs`，另从全新 QA 服务执行 `node tests/ui_selection.mjs`。Windows 自动使用已安装的 Edge；其他环境先在 `frontend/` 执行 `npx playwright install chromium`。测试服务仅监听随机回环端口，使用构造 Emby 数据；关闭终端即可退出，不是插件新增服务。

前端构建输出到插件 `dist/assets/`；`build:preview` 输出到项目父目录 `.work/preview`。本地测试文件和数据不会进入运行组件。

本次 **117 项 pytest、4 个独立回归脚本、31 个浏览器场景**通过。浏览器包括 26 项真实本地 API 流程和 5 项固定诊断响应展示检查。详情见 [TEST_REPORT.md](TEST_REPORT.md)，交接说明见 [HANDOFF.md](HANDOFF.md)。不代表新包已部署或所有播放器、榜单均可用。本包未上传 GitHub、未部署。逐项现场验收见 [ACCEPTANCE.md](ACCEPTANCE.md)。
