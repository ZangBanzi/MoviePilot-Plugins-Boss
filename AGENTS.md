...
当前有效基线是 plugins.v2/mediaarchiver/__init__.py，版本4.6.0。
...
4.6.0 新增六款独立构图，共十款PRESETS。默认布局/适用控件/静态素材数量由PRESETS统一描述；选模板只重置布局配色，不改来源、标题、字体、分辨率或GIF开关。固定构图不展示无效位置控件。方案缩略图是原创示例，运行输出必须继续只用当前库授权素材，不能把示例填入空库。新增模板不得影响已保存的旧方案与8098/302。
